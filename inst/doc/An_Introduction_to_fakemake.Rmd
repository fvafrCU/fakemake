...
Let's take a look at the target rule that builds the list of R code files:
``` {r}
index <- which(sapply(ml, function(x) x["target"] == "log/list_of_r_codes.txt"))
ml[[index]]
```
First, the target rule has no `code` item. Second, its `prerequisites` is a 
character
string that parses as an R expression. Obiously, *fakemake* parses and evaluates 
these character strings dynamically. 
...
```
It is a character vector, the first element being a file name, the other two 
strings containing scripts to evaluate to file names: we can mix both types, 
static and dynamic strings.
If you wonder why the only prerequisite for `testthat` shown in the above graphs 
is `log/list_of_r_codes.txt`: there are no files to be found under `tests/` 
...
Well, they both happen to be part of the list of R codes, so these dependencies 
are redundant, but they are there because the second `prerequisites` for 
`testthat` is "all files under 'tests', recursively":
``` {r}
index <- which(sapply(ml, function(x) x["alias"] == "testthat"))
ml[[index]][["prerequisites"]]
```
